...
@WebServlet(description = "Login Servlet", urlPatterns = { "/LoginServlet" }, initParams = {
		@WebInitParam(name = "user", value = "user"), @WebInitParam(name = "password", value = "pass") })
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (userID.equals(user) && password.equals(pwd) && verify) {
			...
			if (verify) {
				out.println("<div class=\"error\"><font color=red>Either user name or password is wrong.</font></div>");
			} else {
				out.println("<div class=\"error\"><font color=red>You missed the Captcha.</font></div>");
			}
